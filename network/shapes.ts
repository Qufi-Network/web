...
/**
 * An image sampled to points, with an axis along it for something to be read
 * across.
 *
 * Same source format as `fromPoints`, but `u` carries where along the scan a
 * point sits rather than a shuffle — 0 at the right-hand edge, 1 at the left,
 * because the hand this was written for comes in from the right and the reader
 * follows it in.
 */
export function scanned(source: Float32Array, tones: [Vec3, Vec3], scale: number): Shape {
  return (count, rng) => {
    ...
    for (let i = 0; i < total; i += step) {
      ...
      const wanted = source[i * 3 + 2] > 0.5;
      out.push({
        // A little depth, so it is a hand at an angle rather than a decal.
        p: [x * scale, y * scale, (rng() - 0.5) * 1.6],
        sub: wanted ? 1 : 0,
        u: 1 - (x + 1) / 2,
        tone: tones[wanted ? 1 : 0],
      });
    }
    return out;
  };
